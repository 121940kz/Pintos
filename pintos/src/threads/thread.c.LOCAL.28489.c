...
{
  thread_current ()->priority = new_priority;//find max priority in donor list set priority to max of that and new priority
  //t = thread_current()
...
//HLY: Dr. Neilsen asked me to create this function to correctly compute donations 
/*
void recompute_priority(struct thread *t)
{
    intr_disable()//disable interupts 

    //old priority is current priority 
    old_priority = t->priority //HLY: I think this is the current priority, not original one
    ////check if doner list is empty
    if(!list_empty())
    {
        //find maximum list in donation list donars, donated lowerpriority compare_donateing_threads_by_priority

        //t -> priority = donation > default priority  otherwise donation is defalt priority
        //if t priority is greater than old priority && donee is not null
        //then do a recurvsive call 
    }
}
*/
